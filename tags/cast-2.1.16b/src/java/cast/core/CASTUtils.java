...
import cast.cdl.PYTHONSERVERPORT;
...
public class CASTUtils {
	 ...
	 * 
	 * @param <Type>
	 *            Interface type of server
	 * @param <TypePrx>
	 *            Proxy type of interface
	 * @param _componentID
	 *            The ID of the component.
	 * @param _componentHost
	 *            The host of the component.
	 * @param _componentLanguage
	 *            The language the component is written in.
	 * @param _cls
	 *            The class object of Type
	 * @param _prxCls
	 *            The class object of TypePrx
	 * @return
	 * @throws CASTException
	 */
	public static <Type extends Ice.Object, TypePrx extends Ice.ObjectPrx> TypePrx getCASTIceServer(
			String _componentID, String _componentHost,
			ComponentLanguage _componentLanguage, Class<Type> _cls,
			Class<TypePrx> _prxCls) throws CASTException {
		return getCASTIceServer(_componentID, _componentHost,
				_componentLanguage, _cls, _prxCls, Ice.Util.initialize());
	}
	 ...
	 * @throws CASTException
	 */
	static <Type extends Ice.Object, TypePrx extends Ice.ObjectPrx> TypePrx getCASTIceServer(
			String _componentID, String _componentHost,
			ComponentLanguage _componentLanguage, Class<Type> _cls,
			Class<TypePrx> _prxCls, Communicator _communicator)
			throws CASTException {
		...
		try {
			...
			Method checkedCastMethod = helperClass.getMethod("checkedCast",
					Ice.ObjectPrx.class);
			TypePrx checkedPrx = _prxCls.cast(checkedCastMethod.invoke(null,
					prx));
			if (checkedPrx == null) {
				throw new CASTException("Unable to cast returned proxy to "
						+ _prxCls);
			}
			return checkedPrx;
		} catch (Exception e) {
			CASTException ce = new CASTException(
					"Error processing interface class");
			ce.initCause(e);
			throw ce;
		}
	}
	...
	public static ObjectPrx getIceServer(String _componentID, String _category,
		...
		details.append(" -p ");
		details.append(_port);

		ObjectPrx prx = _communicator.stringToProxy(details.toString());
		return prx;
	}

	/**
	 * Concatentate a bunch of strings using a {@link StringBuilder}.
	 * 
	 * @param _strings
	 * @return
	 */
	public static final String concatenate(Object... _strings) {
		StringBuilder sb = new StringBuilder(_strings[0].toString());
	...
	private static ComponentFactoryPrx getComponentFactory(Communicator _ic,
			String _host, int _port) throws ComponentCreationException {
		...
		if (factory == null) {
			...
			// sanity check, factory should have same version string as client!
			String factoryVersion = factory.getVersionString();
			if (!factoryVersion.equals(CASTRELEASESTRING.value)) {
				throw new ComponentCreationException(
						"CAST version mismatch. Client version string: \""
								+ CASTRELEASESTRING.value
								+ "\" but server reports \"" + factoryVersion
								+ "\"");
			}
	...
	public static String toString(WorkingMemoryChange _wmc) {
		...
		buf.append(" ");
		buf.append(Arrays.toString(_wmc.superTypes));
		buf.append(" ");
	...
	public final static int languageToPort(ComponentLanguage _language) {
		...
		} else if (_language == ComponentLanguage.PYTHON) {
			return PYTHONSERVERPORT.value;
		} else {
			throw new RuntimeException("Unknown language: " + _language);
		}
	}

	private static ComponentManagerPrx m_manager;

	public static void setComponentManager(ComponentManagerPrx _man) {
		m_manager = _man;
	}

	public static ComponentManagerPrx getComponentManager() {
		// assert m_manager != null : "component manger has not been set yet";
		return m_manager;
	}
