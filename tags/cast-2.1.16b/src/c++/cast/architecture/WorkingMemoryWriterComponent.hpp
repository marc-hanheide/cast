...
namespace cast {
  ...
  private:
    ...
    ComponentLoggerPtr m_loggerForAdditions;
    ComponentLoggerPtr m_loggerForOverwrites;
    ComponentLoggerPtr m_loggerForDeletes;
     ...
     */
    virtual 
    void logAdd(const std::string & _id, 
                const std::string & _subarch, 
                const std::string & _type,
                const int & _version);
    
    /**
     * Log the overwrote to the logger defined to received
     * overwrites. The logger must be TRACE enabled to receive events.
     * 
     * @param _id
     * @param _subarch
     * @param _type
     ...
     */
    virtual 
    void logOverwrite(const std::string & _id, 
                      const std::string & _subarch, 
                      const std::string & _type,
                      const int & _version);
    
    /**
     * Log the delete to the logger defined to received deletes. The logger must be
     * TRACE enabled to receive events.
     * 
     * @param _id
     * @param _subarch
     */
    virtual 
    void logDelete(const std::string & _id, 
                   const std::string & _subarch);
    
    /**
     * Determines whether the object writted to WM should be copied before write
     */
    bool m_copyOnWrite;
  ...
  protected:
     ...
     */
    bool resetWriteCollocationOptimisation();
  ...
  public:
    ...
    throw (DoesNotExistOnWMException, ConsistencyException, PermissionException, UnknownSubarchitectureException) { 
      ...
      if(m_copyOnWrite) {
        m_workingMemory->overwriteWorkingMemory(_id,_subarch, type, getComponentID(), _data->ice_clone());
      }
      else {
