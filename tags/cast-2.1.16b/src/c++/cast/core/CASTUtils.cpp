...
namespace cast {
  ...
  TimeServerPrx getTimeServer() {
    static TimeServerPrx staticTimeServer;
    if (!staticTimeServer) {
      string host("localhost");
      
      int port = CPPSERVERPORT;
      CommunicatorPtr ic = Ice::initialize();      
      assert(ic);

      Identity id;
      id.name = "TimeServer";
      id.category = "TimeServer";
      ...
      staticTimeServer = TimeServerPrx::checkedCast(base);
    }
    assert(staticTimeServer);

    return staticTimeServer;
  }
...
//   }
  ...
  unsigned int languageToPort(const cdl::ComponentLanguage & _language) {
    if(_language == cdl::CPP) {
      return cdl::CPPSERVERPORT;
    }
    else if(_language == cdl::JAVA) {
      return cdl::JAVASERVERPORT;
    } 
    else if(_language == cdl::PYTHON) {
      return cdl::PYTHONSERVERPORT;
    }
    else {
      throw CASTException("Unknown language: " + _language);
    }
  }
