...
public class SubarchitectureProxies implements CASTConnectionConfiguration {
	...
	void setWorkingMemory(ComponentDescription _config)
		...
		try {
			m_workingMemory = CASTUtils.createWorkingMemory(new Identity(
					...
					_config.hostName, CASTUtils
							.languageToPort(_config.language));
			configureComponent(_config, m_workingMemory);

		} catch (ComponentCreationException e) {
			addComponentLanguageAndRethrow(e, _config.language);
		}
	}
	...
	void addManagedComponent(ComponentDescription _config)
		...
		try {
			ManagedComponentPrx mcp = CASTUtils.createManagedComponent(
					new Identity(_config.componentName, _config.className),
					m_ic, _config.hostName, CASTUtils
							.languageToPort(_config.language));
	...
	private void addComponentLanguageAndRethrow(ComponentCreationException _e,
			ComponentLanguage _language) throws ComponentCreationException {
		ComponentCreationException e = new ComponentCreationException(
				"While creating a " + _language
	...
	public SubarchitectureProxies(SubarchitectureConfiguration _config,
			ComponentManagerPrx _man, Map<String, TimeServerPrx> _servers,
			Communicator _communicator) throws ComponentCreationException {
