...
import cast.cdl.RECEIVERPRIORITYHIGH;
import cast.cdl.RECEIVERPRIORITYLOW;
import cast.cdl.RECEIVERPRIORITYMEDIUM;
...
public abstract class WorkingMemoryReaderComponent extends
	...
	public enum ChangeReceiverPriority {
		HIGH(RECEIVERPRIORITYHIGH.value), MEDIUM(RECEIVERPRIORITYMEDIUM.value), LOW(
				RECEIVERPRIORITYLOW.value);
		public int priority;

		private ChangeReceiverPriority(int _priority) {
			priority = _priority;
		}
	}
	...
	/**
	 * Logger for recording reads
	 */
	private ComponentLogger m_loggerForGets;
	private ComponentLogger m_loggerForUnsubscribedChanges;
	 ...
	 * 
	 * Change receivers can be assigned a priority. This affects the order in
	 * which change receivers associated with the same type of filter within a
	 * single component receive events. This does not influence the order in
	 * which different components receive events. This method assigns medium
	 * priority by default.
	 * 
	...
	public void addChangeFilter(WorkingMemoryChangeFilter _filter,
		...
		addChangeFilter(_filter, _receiver, ChangeReceiverPriority.MEDIUM);
	}
	 ...
	 * which change receivers associated with the same type of filter within a
	 * single component receive events. This does not influence the order in
	 * which different components receive events.
	 * 
	 * For helper functions to construct filters, see
	 * {@link ChangeFilterFactory}.
	 * 
	...
	public void addChangeFilter(WorkingMemoryChangeFilter _filter,
			WorkingMemoryChangeReceiver _receiver,
			ChangeReceiverPriority _priority) {
		addChangeFilter(_filter, _receiver, _priority.priority);
	}
	 ...
	 * @param _receiver
	 *            The receiver object to receive the change.
	 * @param _priority
	 *            An integer used to specify a priority used to order this
	 *            receiver against all other receivers with this filter.
	 * @throws SubarchitectureComponentException
	 */
	public void addChangeFilter(WorkingMemoryChangeFilter _filter,
			WorkingMemoryChangeReceiver _receiver, int _priority) {
		...
		m_changeObjects.put(_filter, _receiver, _priority);

		// println("new filters length: " + m_changeObjects.size());

		m_workingMemory.registerComponentFilter(_filter, _priority);
	}
	...
	void getBaseWorkingMemoryEntries(String _type,
		...
		for (WorkingMemoryEntry entry : holder.value) {
			...
			logGet(entry.id, _subarch, entry.type, entry.version);
			_entries.add(entry);
		}
	}
	...
	public <Type extends Ice.Object> void getMemoryEntries(Class<Type> _cls,
			List<Type> _entries) throws UnknownSubarchitectureException {
	...
	public void removeChangeFilter(
			WorkingMemoryChangeReceiver _workingMemoryChangeReceiver)
			throws SubarchitectureComponentException {
	...
	/**
	 * Blocks until new working memory changes are received. This method will
	 * wait until the workingMemoryChanged function next <b>exits</b>.
	 */
	...
	public void startInternal() {
		...
		m_wmChangeThread = new Thread(m_wmChangeRunnable, getComponentID()
				+ "-receiver");

		m_wmChangeThread.setPriority(Thread.NORM_PRIORITY + 1);
		m_wmChangeThread.start();

		m_loggerForGets = getLogger(".wm.rw.get");
		m_loggerForSubscribedChanges = getLogger(".wm.ch.sub");
		m_loggerForUnsubscribedChanges = getLogger(".wm.ch.all");
	}

	/**
	 * Log the read to the logger defined to received reads. The logger must be
	 ...
	 * @param _version
	 */
	private final void logGet(String _id, String _subarch, String _type,
			int _version) {
	...
	/**
	 * Log a change to the given logger.
	 * 
	 * @param _wmc
	 */
	...
	/**
	 * Log a change which was not subscribed to to the given logger.
	 * 
	 * @param _wmc
	 */
	private void logUnsubscribedChange(WorkingMemoryChange _wmc) {
		if (m_loggerForUnsubscribedChanges.isTraceEnabled()) {
			logChange(_wmc, m_loggerForUnsubscribedChanges);
		}
	}
	 ...
	 * @param _wmc
	 */
	private void logSubscribedChange(WorkingMemoryChange _wmc) {
		if (m_loggerForSubscribedChanges.isTraceEnabled()) {
			logChange(_wmc, m_loggerForSubscribedChanges);
		}
	}
