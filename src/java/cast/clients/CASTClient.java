...
public class CASTClient extends LoggingApplication {
	...
	public int run(String[] args) {
		...
		try {
			...
			for (ComponentDescription cd : CASTConfigParser.m_extras) {
				...
				} else if (cd.language == ComponentLanguage.PYTHON) {
					compPort = PYTHONSERVERPORT.value;
				} else if (cd.language == ComponentLanguage.CPP) {
					compPort = CPPSERVERPORT.value;
				} else {
					throw new RuntimeException("Unknown language value: "
							+ cd.language);
				}
				...
				try {
					...
					assert prx != null : "created proxy is null";
	 ...
	 * 
	 */
	private void storeDescriptions(ComponentManagerPrx _manager) {
		...
		for (SubarchitectureConfiguration sa : CASTConfigParser.m_subarchitectures) {
			ArrayList<ComponentDescription> descriptions = sa.getDescriptions();
			for (ComponentDescription cd : descriptions) {
				m_componentDescriptions.put(cd.componentName, cd);
				_manager.addComponentDescription(cd);
			}
		}
		for (ComponentDescription cd : CASTConfigParser.m_extras) {
			m_componentDescriptions.put(cd.componentName, cd);
