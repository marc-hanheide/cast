...
/**
 * A class to provide additional functionality for those components that are
 * procesing components in a CAAT architecture.
 * 
 * @author nah
 */
public abstract class SubarchitectureComponent extends CASTComponent {
	...
	/**
	 * Overrides the configure method from FrameworkProcess to use _config to
	 * set the subarchitecture ID.
	 * 
	 * @see framework.core.processes.FrameworkProcess#configure(String)
	 * @param _config
	 *            The ID of the subarchitecture which contains this component.
	 */
	@Override
	...
	/**
	 * Gets the logger name currently used by the component. Used during
	 * configure() to retrieve logger for the component.
	 * 
	 * @return
	 */
	@Override
	public String getLoggerName() {
		return getSubarchitectureID() + "." + getComponentID();
	}

	@Override
	public LogAdditions getLogAdditions() {
		return new LogAdditions(getComponentID(), getSubarchitectureID(),
				getStartColourEscape());
	}
