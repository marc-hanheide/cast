...
namespace cast {
  ...
  inline
  void 
  WorkingMemoryWriterComponent::logAdd(const std::string & _id, 
				       const std::string & _subarch, 
				       const std::string & _type,
				       const int & _version) {
    if (m_loggerForAdditions->isTraceEnabled()) {       
      std::ostringstream o;
      o<<"add,"<<getCASTTime()<<","<<_id<<","<<_subarch<<","<<_type<<","<<_version;
      CAST_TRACE(m_loggerForAdditions, o.str() , getLogAdditions());
    }
  }

  inline
  void 
  WorkingMemoryWriterComponent::logOverwrite(const std::string & _id, 
				       const std::string & _subarch, 
				       const std::string & _type,
				       const int & _version) {
    if (m_loggerForOverwrites->isTraceEnabled()) {       
      std::ostringstream o;
      o<<"ovr,"<<getCASTTime()<<","<<_id<<","<<_subarch<<","<<_type<<","<<_version;
      CAST_TRACE(m_loggerForOverwrites, o.str() , getLogAdditions());
    }
  }

  inline
  void 
  WorkingMemoryWriterComponent::logDelete(const std::string & _id, 
					  const std::string & _subarch) {
    if (m_loggerForDeletes->isTraceEnabled()) {       
      std::ostringstream o;
      o<<"del,"<<getCASTTime()<<","<<_id<<","<<_subarch;
      CAST_TRACE(m_loggerForDeletes, o.str() , getLogAdditions());
    }
  }
